...
def protocol(path: Path | None = None) -> dict[str, Any]:
    ...
    if schema.endswith("/v1"):
        ...
        expected_steps = [750] if schema.endswith("/v7") else [5000] if schema.endswith("/v9") else [1000]
        ...
        if schema.endswith("/v9"):
            require(approval.get("exposure_matched_control_approved") is True, "Exposure-matched control is not approved")
