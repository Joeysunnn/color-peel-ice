...
| Change only `<c2*>` initializer to locked `turquoise` | selected by user review; dual smoke and full training pending | Scientific ablation against the frozen baseline; CAA, AdamW, mask, prompt, data, seed, steps, and other initializers stay fixed |
...
| Disable confirmed false-positive SafetyChecker for matched turquoise evaluation | selected for the new 900-image run; paired acknowledgement required and recorded per manifest row | Prevents post-generation all-zero replacement; changes generation semantics but not training or the frozen baseline |
