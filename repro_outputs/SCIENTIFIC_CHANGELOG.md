...
| Replace cross-run raw-byte equality with decoded-pixel equivalence | v1 retained; v2 explicitly approved; 180/180 comparisons passed and realization completed | Realization-gate correction only: RGB mean and changed-channel fraction <=0.001; max is record-only; masks exact; raw hashes remain evidence; render requests and parameters unchanged |
...
| Calibrate Qwen material labels on accepted v3 references | completed 360/360; GT rubber only 38/180 | Evaluation-only diagnostic using the identical frozen Qwen prompt; establishes evaluator limitation, not model success |
